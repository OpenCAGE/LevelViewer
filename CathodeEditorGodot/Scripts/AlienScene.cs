...
public partial class AlienScene : Node3D
{
    ...
    private void CreateRenderable(Node3D parent, Models.CS2.Component.LOD.Submesh submesh, Materials.Material material)
    {
        ...
        StandardMaterial3D godotMaterial = GetMaterial(material);
        if (!_materialSupport[godotMaterial])
            return;

        MeshInstance3D meshInstance = new MeshInstance3D
        {
            Name = holder.MainMesh.ResourceName + " (" + godotMaterial.ResourceName + ")",
            Mesh = holder.MainMesh,
            MaterialOverride = godotMaterial,
            Visible = _materialSupport[godotMaterial],
        };
    ...
    private StandardMaterial3D GetMaterial(Materials.Material material)
    {
